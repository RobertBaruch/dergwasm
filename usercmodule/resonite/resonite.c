...
mp_obj_t resonite_create_slot(resonite_slot_t reference_id);
...
typedef struct _resonite_Slot_obj_t {
    ...
    resonite_slot_t reference_id;
} resonite_Slot_obj_t;

// This represents Slot.__new__ and Slot.__init__.
STATIC mp_obj_t resonite_Slot_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *args) {
    ...
    resonite_Slot_obj_t *self = mp_obj_malloc(resonite_Slot_obj_t, type);
    // args[0] is reference_id_lo
    self->reference_id = (unsigned long)mp_obj_get_int(args[0]); // mp_int_t is an int under the WASM port.
    // args[1] is reference_id_hi
    self->reference_id |= (unsigned long)mp_obj_get_int(args[1]) << 32; // mp_int_t is an int under the WASM port.
    return MP_OBJ_FROM_PTR(self);
}

STATIC mp_obj_t resonite_Slot_get_parent(mp_obj_t self_in) {
    resonite_Slot_obj_t *self = MP_OBJ_TO_PTR(self_in);
    resonite_slot_t parent_id = slot__get_parent(self->reference_id);
...
// This defines the mp_obj_type_t for the Slot object, called resonite_type_Slot.
...
mp_obj_t resonite_create_slot(resonite_slot_t reference_id) {
    resonite_Slot_obj_t *self = mp_obj_malloc(resonite_Slot_obj_t, &resonite_type_Slot);
...
STATIC const mp_rom_map_elem_t resonite_module_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_add_ints), MP_ROM_PTR(&resonite_add_ints_obj) },
    { MP_ROM_QSTR(MP_QSTR_Slot),     MP_ROM_PTR(&resonite_type_Slot) },
};
