...
namespace Derg.Mem
{
    public readonly struct TerminatedUtf8Marshaller : IMarshaller<string>
    {
        ...
        private unsafe Buffer<byte> PutString(string obj, in MemoryContext ctx)
        {
            // Allocate the string.
            var buffer = ctx.Allocate(Encoding.UTF8.GetByteCount(obj) + 1);
            var bytes = Encoding.UTF8.GetBytes(obj);
            var bufferMem = ctx.BufferView(buffer);
            bufferMem.CopyFrom(bytes);
            bytes[bytes.Length - 1] = 0; // Null terminator
            return buffer;
        }

        private unsafe string GetString(Pointer<byte> ptr, in MemoryContext ctx)
        {
